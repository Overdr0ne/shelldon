...
#+begin_src elisp
    (use-package shelldon
      :straight (shelldon :type git
        ...
        ; optionally disable shelldon to automatically complete your history 
        ; (setopt shelldon-autohistory-p nil)
        ; optionally disable desktop notifications
...
| shelldon-async-command                                         | enter asynchronous shell command in the minibuffer |
...
** minibuffer
- By default, shelldon automatically completes your from your shell history. You may disable this feature by setting `shelldon-autohistory-p` to nil.

- In any case, you may of course search the minibuffer history for *shelldon* commands, as with any comint command, with `comint-history-isearch-backward` (M-r) and the like.

- It is often useful to run a shell command and then paste the output later. The `shell-command-kill-output` will also copy the output of the shell command to the `kill-ring`.
